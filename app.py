...
if page == "About Me":
    ...
    with col1:
        st.markdown("""
        Hi, I’m **Alok Mahadev Tungal** — a passionate **Data Scientist** and **Machine Learning Enthusiast**.
        This app demonstrates building a complete project from *data analysis* ➝ *visualization* ➝ *model building* ➝ *deployment*.
        """)
        st.markdown("""
        - **LinkedIn:** [https://www.linkedin.com/](https://www.linkedin.com/)
        - **GitHub:** [https://github.com/](https://github.com/)
...
# --- PAGE 3: Data Insights ---
    ...
    if plot_choice == 'Distribution of Car Prices':
        ...
        df_plot = pd.DataFrame({'brand': brands, 'price': prices})
        top_brands = df_plot['brand'].value_counts().nlargest(8).index
        for brand in top_brands:
            fig.add_trace(go.Box(y=df_plot[df_plot['brand']==brand]['price'], name=brand, marker_color='#facc15'))
        fig.update_layout(title='<b>Price Distribution by Brand</b>', showlegend=False)
    elif plot_choice == 'Feature Correlation Heatmap':
        corr_matrix = [[1.00, -0.65, -0.45], [-0.65, 1.00, 0.30], [-0.45, 0.30, 1.00]]
        labels = ['Price', 'Age', 'KM']
